...
7. Admin staff management (super admin model): **Aurora Admin** (`maileditorportfolio@gmail.com`, users id#1) is the only **super admin** (`is_super_admin=1`). Users table has `is_active` + `is_super_admin` columns (added to `setup.sql` AND live via one-off migration).
   ...
   - **Mail identity:** admin-triggered emails (order status, payment request/paid, customer email single+bulk, customer create, admin create) stamp the acting admin — `admin_identity()` helper in `includes/auth.php` (reads name/email from DB by session), `send_mail()` gains a 6th `$sentBy` arg (From display name `MAIL_FROM_NAME · <admin>`, `Reply-To` = admin, brevo `replyTo`), `email_layout()` renders a "Sent by <admin> · <email>" footer line via `"sent_by"` opt. Envelope sender stays MAIL_FROM (Gmail SMTP requires authenticated account = From; a per-admin Gmail app-password flow is a proposed future option, NOT implemented). Live-verified: regular admin status/payment-request/customer-email sends go out with identity; render probe shows the line.
