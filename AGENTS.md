...
7. Admin staff management: `admin/admins.php` (list all `role='admin'` users) + `api/admin/admin_create.php` (guarded by `admin-guard.php` → returns 401 to non-admins) + `assets/js/admin-admins.js` (modal, email of credentials to the new admin). Nav item `admins` added in `admin/inc/head.php`, i18n key `a_admins` added in `assets/js/i18n.js`. **ONLY an existing admin can create another admin** (page `require_admin()`, API `is_admin()` check). Admins show in the Customers list with an ADMIN badge and are NOT deletable there. Verified live: create → new admin can sign in to the panel and reach the Admins page; a customer hitting the create API gets 401.
